...
//int LTEMP = 0; // temp count for strings
...
int Count_Strings(ASTnode *p){
  int c = 0;
  if(p==NULL) return(0);
  if(p->type == A_CONSTANT_STRING) c = 1;
  return(c + Count_Strings(p->S1) + Count_Strings(p->S2) + Count_Strings(p->next));
}
...
//   return(strdup(s));
// }
// do again for labels and whatev